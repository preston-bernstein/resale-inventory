---
name: internal-inventory-app-failure-archaeology
description: Historical record of every known defect, dead-end, rejected design, and near-miss in the internal-inventory-app repo (formerly internal-inventory-app). Consult when asking "has this been tried before", "why is X like this", "known issues", "past bugs", "is this a known problem", "why is there no gross_profit column", "why did import return 500", or before proposing a design change or filing a new bug. Also the single place to APPEND new failure entries.
---

# Book-Seller Failure Archaeology

The institutional memory of what went wrong, what almost went wrong, and what was
deliberately rejected in `/Users/prestonbernstein/dev/internal-inventory-app`. Read the relevant
...
|---|---|
| `docs/book-inventory-management/requirements.md`, `plan.md`, `steps.md` | Current spec — the change-control authority for this feature (per coordinator decision; see sibling skill `internal-inventory-app-change-control`) |
...
```sh
cd /Users/prestonbernstein/dev/internal-inventory-app/docs/book-inventory-management
...
(`internal-inventory-app-validation-and-qa` owns the procedure) — none of this tooling existed
...
- **Routed-to**: Rejected-designs register. Schema questions → `internal-inventory-app-architecture-contract`.
...
- **Status**: FIXED-BY-DESIGN.
- **Routed-to**: `internal-inventory-app-architecture-contract` for DB-layer invariants.
...
- **Routed-to**: `bookselling-domain-reference` for the status lifecycle; `internal-inventory-app-change-control` before editing FR10.
...
- **Routed-to**: `internal-inventory-app-validation-and-qa` for review process; this file for the D1/D2 fallout.
...
- **Routed-to**: `internal-inventory-app-change-control` (this is a spec change, not a bug fix).
...
- **Routed-to**: `internal-inventory-app-constraint-leak-campaign`.
...
- **Routed-to**: `internal-inventory-app-constraint-leak-campaign` (fix alongside D1 — same constraint).
...
- **Routed-to**: `internal-inventory-app-build-and-env`, `internal-inventory-app-run-and-operate`.
...
- **Root cause**: `006_tenant_scoping.sql`'s six `ALTER TABLE ... ADD COLUMN tenant_id TEXT NOT NULL REFERENCES tenants(id) DEFAULT '...'` statements are individually valid SQL (isolated single-connection repro against a throwaway file, both with `foreign_keys = ON` and `.immediate()`-wrapped exactly like `lib/db.ts`, succeeded cleanly every time — see reproduction below). The error only appears when **two separate OS processes** (e.g. `next build`'s Turbopack workers, or the `next dev` webServer process racing a spec file's own `require('@/lib/db')`) each independently run the full `VERSIONED_MIGRATIONS` loop against the same not-yet-migrated file at once. `db.transaction(fn).immediate()` serializes the two racers via SQLite's write lock, but the loser's connection observes a schema state mid-DDL from the winner in a way that makes SQLite reject the ADD COLUMN's REFERENCES+DEFAULT combination on retry — this is the same underlying design gap the build-time race gotcha documented in `project_resale_inventory_tour_deploy.md` (Obsidian memory, book-seller/internal-inventory-app deploy session) already named for older migrations (there it surfaced as `no such column: book_id` / `table items already exists`; same root cause, different SQLite error string depending which migration and which statement loses the race).
...
- **Routed-to**: was `internal-inventory-app-constraint-leak-campaign`/hardening; fixed per Task 23 in TASKS.md.
...
- **Why `db.backup()` not `cp`**: recent committed writes live in `inventory.db-wal` until checkpoint; a plain file copy of `inventory.db` alone can silently lose them (see `internal-inventory-app-run-and-operate` data topology). The online-backup API takes a consistent snapshot across the WAL.
- **Routed-to**: `internal-inventory-app-run-and-operate` (operational safety).
...
- **Routed-to**: `internal-inventory-app-architecture-contract` (error-signaling contract for `lookupISBN`).
...
- **Routed-to**: was `internal-inventory-app-build-and-env`, `internal-inventory-app-config-and-constants`.
...
- **Routed-to**: `internal-inventory-app-architecture-contract` if anyone proposes "fixing" it — check it is actually hurting first.
...
- **Routed-to**: `internal-inventory-app-architecture-contract` for the current type shapes.
...
- **Fix constraint**: the schema declares `previous_price INTEGER NOT NULL` (`data/migrations/001_init.sql`), so "write NULL instead" requires a schema migration (table rebuild — `internal-inventory-app-change-control` §4) or a redesigned sentinel; the route-level `?? 0` is a coerced choice, not a free one.
...
- **Routed-to**: was `internal-inventory-app-constraint-leak-campaign` if batched with D1/D3 (same file); fixed standalone via `internal-inventory-app-change-control` (Task 26).

### D4 | 2026-07-03 | POST /api/books/:id/status Sold response omits gross_profit

- **Symptom**: `AC3: Sale Pending → Sold; gross_profit = sale_price - acquisition_cost` (`tests/integration.test.ts`, HTTP suite) fails with `expected undefined to be 1000` — discovered while temporarily activating the HTTP suite in a scratch copy to verify the D1/D2/D3 regression tests added by `internal-inventory-app-constraint-leak-campaign` (2026-07-03). Never caught before because the HTTP suite has always been `describe.skip`.
...
- **Routed-to**: was `internal-inventory-app-change-control` (behavior-changing); gated and fixed per Task 20 in TASKS.md.
...
- **Routed-to**: was `internal-inventory-app-validation-and-qa` (test correctness); fixed per Task 20 in TASKS.md.
...
This file is the **single home** for failure history; `internal-inventory-app-debugging-playbook`
...
- DB reads only via `sqlite3 "file:/Users/prestonbernstein/dev/internal-inventory-app/data/inventory.db?mode=ro" "..."`. Never modify/delete/recreate `data/inventory.db` or its `-wal`/`-shm`.
- `npx vitest run` is now safe against the real DB by default (T1, fixed) — but `npm run build` / `npm run dev` / `npm start` are **not**; they still open and, if pending, migrate the real DB. See `internal-inventory-app-build-and-env` CRITICAL SAFETY WARNING #2.
...
- **Triage of a live, unlisted symptom** — start with `internal-inventory-app-debugging-playbook`; come back here to check for priors and to append the outcome.
- **Fixing D1/D2/D3** — the active fix plan lives in `internal-inventory-app-constraint-leak-campaign`; this file only records their history.
- **"How do I run/build/test the app?"** — `internal-inventory-app-build-and-env`, `internal-inventory-app-run-and-operate`.
- **Current architecture or schema questions** ("what IS the design", not "why did it change") — `internal-inventory-app-architecture-contract`.
- **Domain semantics** (what Sale Pending means for a bookseller) — `bookselling-domain-reference`.
- **Proposing/authorizing a spec change** — `internal-inventory-app-change-control`; `docs/book-inventory-management/` is the change-control authority, not this file.
- **General QA method, tooling, or research** — `internal-inventory-app-validation-and-qa`, `internal-inventory-app-diagnostics-and-tooling`, `internal-inventory-app-research-frontier`, `internal-inventory-app-analysis-and-methodology`.
...
fixed, D3 confirmed live then fixed, per `internal-inventory-app-constraint-leak-campaign`. D4 and
...
- Git history exists now: `git -C /Users/prestonbernstein/dev/internal-inventory-app log --oneline | tail -1` (expect `2ebb1ae Initial commit: book inventory management app`, with 20+ commits on top — NOT "does not have any commits yet"; that was only ever true before 2026-07-03)

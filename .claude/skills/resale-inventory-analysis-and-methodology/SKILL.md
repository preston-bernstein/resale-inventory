...
**Headline (as run 2026-07-02): ~12 rule groups; 3 uncovered (rule, route) pairs; all 3 corresponded exactly to the live defect cluster.** The audit's negative space WAS the bug list. Fix record: `internal-inventory-app-failure-archaeology` (D1-D4). Note the unique index was, and its successor `idx_book_details_isbn` still is, *partial* (`WHERE isbn IS NOT NULL`) — always check the migration file directly for CHECK/index clauses; a plan.md copy can omit nuances.
...
**When:** any proposed change to statuses or transitions (which is spec-gated — FR10, `internal-inventory-app-change-control`).
...
```
proposal → spec gate (internal-inventory-app-change-control)
        → experiment with pre-registered predictions (method 4)
        → adopt: spec + code + tests land together
        → or retire: dated entry in internal-inventory-app-failure-archaeology (rejected register)
```
...
- Executing the defect fix → `internal-inventory-app-constraint-leak-campaign` (it consumes method 1's output).
- What evidence a change class needs procedurally → `internal-inventory-app-validation-and-qa` / `internal-inventory-app-change-control`.
- Triage of a live failure → `internal-inventory-app-debugging-playbook`.
- Historical entries themselves → `internal-inventory-app-failure-archaeology`.
...
- D1-D4 status: `grep -n "^| D[1-4] " .claude/skills/internal-inventory-app-failure-archaeology/SKILL.md` (expect all FIXED; if any regressed, that's the next audit's finding).

---
name: internal-inventory-app-build-and-env
description: Build and environment setup runbook for the internal-inventory-app repo (formerly internal-inventory-app; Next.js 15 + better-sqlite3 local-first inventory app). Use when asked to "set up" the project, "install" dependencies, "clone" onto a "fresh machine", run "npm ci", fix a "build fails" error, resolve "node version" / native-module ABI problems (ERR_DLOPEN), or do a clean rebuild. Ends when `npm run build` is green — running/operating and testing are sibling skills.
---

# internal-inventory-app — Build and Environment

Scope: get from a bare checkout to a green `npm run build` on this machine. Nothing more.
When the build is green, hand off to `internal-inventory-app-run-and-operate` (running the app, DB care)
or `internal-inventory-app-validation-and-qa` (testing).

Repo root (always run commands from here): `/Users/prestonbernstein/dev/internal-inventory-app`
...
1. **`npm test` / `npx vitest run` / `npm run test:coverage` are SAFE BY DEFAULT now —
   ...
   **Do not remove or override those env vars in either config** — that is the entire
   ...
   `internal-inventory-app-failure-archaeology`.
...
```sh
cd /Users/prestonbernstein/dev/internal-inventory-app
```
...
- If `node_modules/` already exists and you just want to confirm it is in sync, use the
  ...
  traps" and `internal-inventory-app-validation-and-qa` for what runs them.
...
Database initialized at: /Users/prestonbernstein/dev/internal-inventory-app/data/inventory.db
...
`/inventory*` (see `internal-inventory-app-architecture-contract` for the full route/table map).
...
- [ ] Testing itself (running the suite, coverage, mutation, E2E) is safe-by-default now
      ...
      thresholds, and commands live in `internal-inventory-app-validation-and-qa`.

Build green? You are done. Running the app is `internal-inventory-app-run-and-operate`.
...
module load, in order (source: `/Users/prestonbernstein/dev/internal-inventory-app/lib/db.ts`):
...
`/Users/prestonbernstein/dev/internal-inventory-app/data/inventory.db`, you created a stray.

Detection command (finds stray `data/inventory.db` files under home, excluding the real
one and noise dirs):

```sh
find ~ -maxdepth 4 -path "*/data/inventory.db" -not -path "*/dev/internal-inventory-app/*" -not -path "*/node_modules/*" 2>/dev/null
```

Expected output: nothing. Any hit is a stray — safe to delete **only after confirming it
is not the real DB** at `/Users/prestonbernstein/dev/internal-inventory-app/data/inventory.db`.
...
- **Running or operating the app** (dev server lifecycle, prod start, DB care, backups) →
  `internal-inventory-app-run-and-operate`. This skill ends when `npm run build` is green.
- **Running tests** (unit/component via vitest, E2E via Playwright, mutation via Stryker,
  coverage thresholds) → `internal-inventory-app-validation-and-qa`.
- **Diagnosing app bugs / runtime misbehavior** → `internal-inventory-app-debugging-playbook`,
  `internal-inventory-app-diagnostics-and-tooling`.
- **Understanding architecture, routes, data model** → `internal-inventory-app-architecture-contract`.
- **Changing config values or constants** → `internal-inventory-app-config-and-constants` (and
  `internal-inventory-app-change-control` for any change process).
- **Past incidents / why things are the way they are** → `internal-inventory-app-failure-archaeology`.
- **Domain questions about bookselling itself** → `bookselling-domain-reference`.
- **Docs/writing, research, analysis** → `internal-inventory-app-docs-and-writing`,
  `internal-inventory-app-research-frontier`, `internal-inventory-app-analysis-and-methodology`.
...
skipped — matches `internal-inventory-app-failure-archaeology`'s baseline) to confirm the real DB
...
cd /Users/prestonbernstein/dev/internal-inventory-app && npm ls --depth=0  # dep tree vs lockfile
cd /Users/prestonbernstein/dev/internal-inventory-app && npm run build     # route table still 17 routes? (touches the real DB — see CRITICAL SAFETY WARNING #2)
file /Users/prestonbernstein/dev/internal-inventory-app/node_modules/better-sqlite3/build/Release/better_sqlite3.node  # native binary arch
grep -n "process.cwd()\|BOOKSELLER_DB_PATH" /Users/prestonbernstein/dev/internal-inventory-app/lib/db.ts  # cwd trap + env override still present?
sed -n '135,145p' /Users/prestonbernstein/dev/internal-inventory-app/tests/integration.test.ts  # wipe-on-scratch-DB behavior still present?
grep -n "BOOKSELLER_DB_PATH\|BOOKSELLER_PHOTOS_PATH" /Users/prestonbernstein/dev/internal-inventory-app/vitest.config.ts /Users/prestonbernstein/dev/internal-inventory-app/playwright.config.ts  # safe-by-default wiring still present?
sqlite3 "file:/Users/prestonbernstein/dev/internal-inventory-app/data/inventory.db?mode=ro" "PRAGMA user_version;"  # read-only DB schema-version check, never write
```

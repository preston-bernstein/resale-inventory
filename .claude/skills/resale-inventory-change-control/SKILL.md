---
name: internal-inventory-app-change-control
description: Governs how changes are made to the internal-inventory-app repo (formerly internal-inventory-app) — spec-first authority order, change classification gates, non-negotiable rules, schema-migration protocol, and git/commit policy. Load before editing any code, spec, or schema in this repo; when asked "can I change X", "how do I add a status/condition/column", "should I update requirements.md first", "can I commit this", or before any behavior-changing, schema, or security-touching edit.
---

# internal-inventory-app Change Control

Runbook for making any change to `/Users/prestonbernstein/dev/internal-inventory-app` — a local-first multi-category (books + clothing) resale inventory app (Next.js 15 App Router + better-sqlite3, sole operator, no CI, has lint (`npm run lint` via ESLint), has git history). This skill defines WHO decides, WHAT order changes happen in, and WHICH rules are never broken. It does not teach the architecture (see `internal-inventory-app-architecture-contract`) or how to debug (see `internal-inventory-app-debugging-playbook`).
...
- **Constraint leak** = a DB CHECK constraint violation surfacing as HTTP 500 instead of a validated 422. This was the project's live defect cluster (D1/D2/D3/D4) — **all four are now FIXED** (2026-07-03; see `internal-inventory-app-constraint-leak-campaign` for the completed campaign and `internal-inventory-app-failure-archaeology` for the fix record). The pattern and its fix methodology remain the reference for any *new* constraint-leak defect.
...
There is no CI, no PR pipeline. ESLint (`npm run lint`) exists but isn't a gate this section enumerates separately — treat a clean `npm run build` as covering it unless a change specifically touches lint config. "Merge-equivalent" here means: the evidence below exists before you call the change done. HTTP-level verification means a GET-only probe, the safe-test procedure from `internal-inventory-app-validation-and-qa`, or — when the behavior under test is itself mutating — a gated mutating probe under a campaign-style protocol (operator backup first, `CAMPAIGN-PROBE`-tagged rows, verified cleanup; the reference protocol is `internal-inventory-app-constraint-leak-campaign`). Never ad-hoc mutating requests against the live DB, and **never a raw `npx vitest run`** (see non-negotiable (e)).
...
| d | Never weaken or drop DB CHECK constraints to silence HTTP 500s | The CHECKs are the last line of data integrity; the 500s are a validation-layer gap, not a schema bug. Correct fix = validate before write, return 422 | Historical: four verified constraint leaks (D1/D2/D3/D4, 2026-07-02/03) — status→Listed without `listing_price`; CSV import with duplicate ISBN losing the whole batch; PATCH `listing_price:null` on a Listed item; Sold response omitting `gross_profit`. All FIXED 2026-07-03 with route-level pre-validation, not constraint-weakening. Campaign record: `internal-inventory-app-constraint-leak-campaign` |
| e | Never run `npx vitest run` (or any full test run) without the safe procedure in `internal-inventory-app-validation-and-qa` | `tests/integration.test.ts` (~line 139) executes `DELETE FROM item_photos; DELETE FROM price_history; DELETE FROM item_platforms; DELETE FROM clothing_details; DELETE FROM book_details; DELETE FROM items;` — this trap is now mitigated (not eliminated) by `BOOKSELLER_DB_PATH`/`BOOKSELLER_PHOTOS_PATH` env vars (`lib/db.ts`, `lib/photos.ts`) which `vitest.config.ts` sets to scratch locations for every test run; a raw `vitest run` invoked OUTSIDE that config (or any ad hoc script importing `lib/db.ts` without the env vars set) still resolves to `data/inventory.db` via `process.cwd()` and wipes it | Verified against current `tests/integration.test.ts` and `vitest.config.ts` |
...
5. Before any migration touches a DB with real data: take a backup first (`lib/backup.ts` runs one automatically on startup, but don't rely on timing — take an explicit one; procedure in `internal-inventory-app-run-and-operate`). Sessions never migrate a real/live DB autonomously.
...
[ ] If the change area is a known defect (constraint-leak 500s, import duplicate-ISBN),
    read internal-inventory-app-constraint-leak-campaign first — don't fix ad hoc
```
...
[ ] Safe-test procedure from internal-inventory-app-validation-and-qa executed — NOT raw npx vitest run
...
- **Understanding the design, invariants, or why the system is shaped this way** → `internal-inventory-app-architecture-contract`
- **A symptom is in front of you and you need triage** → `internal-inventory-app-debugging-playbook`
- **History of a specific past defect** → `internal-inventory-app-failure-archaeology`
- **The constraint-leak methodology** (D1-D4 are FIXED, but the campaign is the reference for diagnosing a *new* CHECK-constraint-vs-route-validation gap) → `internal-inventory-app-constraint-leak-campaign` (this skill only tells you the gates around that kind of work)
- **How to test safely / what evidence counts** → `internal-inventory-app-validation-and-qa`
- **Running the app, DB backups, operational care** → `internal-inventory-app-run-and-operate`
- **Build, environment, Node/Next specifics** → `internal-inventory-app-build-and-env`
- **Where a constant lives / config values** → `internal-inventory-app-config-and-constants`
- **Writing docs or spec-file templates** → `internal-inventory-app-docs-and-writing`
- **Book-domain questions** (condition grades, platforms) → `bookselling-domain-reference`
- **Diagnostic scripts and tooling** → `internal-inventory-app-diagnostics-and-tooling`
...
- Git history exists: `cd /Users/prestonbernstein/dev/internal-inventory-app && git log --oneline | wc -l` (expect a real count, not a "does not have any commits yet" error)
- Versioned migration runner: `grep -n "VERSIONED_MIGRATIONS" /Users/prestonbernstein/dev/internal-inventory-app/lib/db.ts` (expect the array, keyed off `PRAGMA user_version`)
- Migration count: `ls /Users/prestonbernstein/dev/internal-inventory-app/data/migrations/` (expect `001_init.sql`, `002_price_history_nullable.sql`, `003_multi_category.sql` — a 4th file means a schema change shipped since this was last checked)
- Test-suite DB-wipe trap still present (but mitigated by env-var redirection): `grep -n "DELETE FROM" /Users/prestonbernstein/dev/internal-inventory-app/tests/integration.test.ts` (any hit = trap live in that file; confirm `vitest.config.ts` still sets `BOOKSELLER_DB_PATH`/`BOOKSELLER_PHOTOS_PATH` to scratch paths — do NOT run the suite to check)
- CSRF middleware present: `cat /Users/prestonbernstein/dev/internal-inventory-app/middleware.ts` (expect an Origin-header check on mutating methods, matcher `/api/:path*`)
- Startup backup routine present: `cat /Users/prestonbernstein/dev/internal-inventory-app/lib/backup.ts` (expect `runStartupBackup`, 7-file retention, `db.backup()`)
- AC3 contradiction still open: `grep -n "AC3" /Users/prestonbernstein/dev/internal-inventory-app/docs/book-inventory-management/challenge-notes.md` (open question section) — cross-check current status in `internal-inventory-app-failure-archaeology` (SR-6)
- Route surface: `find /Users/prestonbernstein/dev/internal-inventory-app/app -name "page.tsx" -o -name "route.ts"` (expect items/dashboard/export/import/isbn API routes and inventory/dashboard/playbook pages under app/ — no app/api/books or app/books)
- Transition map unchanged: `grep -n "Sale Pending" /Users/prestonbernstein/dev/internal-inventory-app/lib/transitions.ts` (Listed→Sold must NOT appear as allowed)

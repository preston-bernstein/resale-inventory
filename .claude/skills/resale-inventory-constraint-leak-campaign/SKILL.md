...
|---|---|---|---|---|
| 0.1 | Build green | `npm run build` | Route table with 13 routes, no errors | STOP — fix build first (`internal-inventory-app-build-and-env`) |
| 0.2 | Operator backup taken | `sqlite3 data/inventory.db ".backup 'data/backups/inventory-$(date +%Y%m%d)-pre-campaign.db'"` then `sqlite3 "data/backups/..." "PRAGMA integrity_check;"` → `ok` | Backup exists, integrity ok | STOP — no mutation without backup (`internal-inventory-app-run-and-operate`) |
| 0.3 | Server up, real port known | `npm run dev` (or `-H 127.0.0.1 -p 3005`), then `.claude/skills/internal-inventory-app-diagnostics-and-tooling/scripts/find-port.sh` | Prints a port; set `B=http://127.0.0.1:<port>` | Port trap — never probe :3000 blind |
| 0.4 | Baseline tests green | Scratch-copy procedure B in `internal-inventory-app-validation-and-qa` | `139 passed | 15 skipped` (+N for tests added since 2026-07-02) | STOP — investigate first |
| 0.5 | DB invariants clean | `.claude/skills/internal-inventory-app-diagnostics-and-tooling/scripts/db-integrity.sh` | `DB-INTEGRITY: clean` | STOP — record in failure-archaeology |
...
curl -s -w "\nHTTP %{http_code}\n" -X POST $B/api/import \
  -F "file=@.claude/skills/internal-inventory-app-constraint-leak-campaign/fixtures/dup-isbn.csv"
...
**Verify D3 now** (prediction first — `internal-inventory-app-analysis-and-methodology`): predict `HTTP 500` when PATCHing `{"listing_price": null}` on a Listed book. Create a second probe book, PATCH it a price, transition it to Listed, then:
...
**Before writing code:** classify via `internal-inventory-app-change-control` (behavior-changing — API status codes change from 500 to 4xx) and update `plan.md`'s API contract (and requirements if AC wording moves) FIRST.
...
- **Theory obligations:** every DB rule that a route can trigger gets a route-level guard producing a 4xx — run the constraint-coverage audit (`internal-inventory-app-analysis-and-methodology`) after coding to prove no rule is left uncovered.
...
|---|---|
| Weaken/drop the CHECK constraints | The DB is the last line of integrity; non-negotiable (`internal-inventory-app-change-control` §3, `internal-inventory-app-architecture-contract`) |
...
4. **New HTTP regression tests added** following `internal-inventory-app-validation-and-qa` §how-to-add (configurable base URL, defect IDs in test names).
...
- You just saw a 500 and don't know it's this cluster yet → `internal-inventory-app-debugging-playbook` (triage first).
- General "how do I make changes here" → `internal-inventory-app-change-control`.
- Running tests safely → `internal-inventory-app-validation-and-qa`.
...
- D1-D4 current status: `internal-inventory-app-failure-archaeology` (expect all four FIXED; if any shows a different status, something regressed).

---
name: internal-inventory-app-docs-and-writing
description: Where internal-inventory-app's (formerly internal-inventory-app) documentation lives, which file wins, the house writing style, and copy-paste templates for new feature specs, task trackers, and review notes. Use when asked to "write a spec", "create new feature docs", "update requirements", "edit TASKS.md", "document a decision", "fix the README", or when unsure where a piece of documentation belongs or how to phrase an FR/AC.
---
...
> ASSUMPTION (coordinator-approved): `docs/<slug>/` spec folders are the change-control authority. Gates and precedence live in `internal-inventory-app-change-control` — this skill covers the files themselves and how to write them.
...
| `docs/<slug>/*.bak` | Frozen pre-review originals of requirements/plan/steps | historical | **NEVER edit.** Their only use: diff against current to see what the review changed (`internal-inventory-app-failure-archaeology` does this) |
...
The "Open questions" section is load-bearing: the AC3 contradiction lives in one (SR-6 in `internal-inventory-app-failure-archaeology`) and is still governing behavior. Never delete an open question — resolve it with a dated answer or carry it.
...
- New failure knowledge is APPENDED to `internal-inventory-app-failure-archaeology` (it has the entry template), then linked.
- Descriptions must say when to load the skill (trigger phrases) and when not — models route on descriptions alone.

## When NOT to use this skill

- What gate a change needs / whether spec must move first → `internal-inventory-app-change-control`.
- Recording a defect or investigation → `internal-inventory-app-failure-archaeology` (its own template).
- Writing tests → `internal-inventory-app-validation-and-qa`.

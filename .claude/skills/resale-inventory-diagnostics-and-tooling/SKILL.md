...
Run from anywhere; scripts default to the repo at `/Users/prestonbernstein/dev/internal-inventory-app`:

```bash
cd /Users/prestonbernstein/dev/internal-inventory-app
.claude/skills/internal-inventory-app-diagnostics-and-tooling/scripts/find-port.sh
```
...
```
internal-inventory-app not found on ports 3000-3010 (is the server running? npm run dev)
```
(exit 1)
...
| Error line, exit 1 | Not serving on 3000–3010 | `npm run dev` (see `internal-inventory-app-run-and-operate`); if you believe it IS running, check its startup log for the real port or `lsof -nP -iTCP -sTCP:LISTEN \| grep -i node` |
...
Five GET-only probes against the current routes, each verified against route source before scripting. Requires a server running (start one per `internal-inventory-app-run-and-operate`, or let this script auto-detect via `find-port.sh`); it does not start one itself.
...
|---|---|---|
| dashboard shape | Wrong target (another app's HTML), or dashboard route changed | `find-port.sh`; then `internal-inventory-app-debugging-playbook` |
| items envelope | Route contract changed without spec — that's a gated change | `internal-inventory-app-change-control` |
| limit=999 not 400 | Bounds validation removed/altered in `app/api/items/route.ts` | `internal-inventory-app-config-and-constants` ledger + change-control |
| ISBN 400 probe | Pattern validation changed (2 homes — may have drifted apart) | `constants-drift.sh`, then ledger |
| export headers | Export route changed; CSV consumers (import round-trip!) at risk | `internal-inventory-app-change-control` |

Note: the smoke suite deliberately contains **no probe for the old D1/D2 constraint-leak 500s** — those required mutating requests to trigger, and both are fixed now anyway (see `internal-inventory-app-debugging-playbook`). Their historical gated reproduction protocol lived in `internal-inventory-app-constraint-leak-campaign`.
...
|---|---|---|
| integrity_check != ok | File corruption | STOP; operator restores from backup (`internal-inventory-app-run-and-operate`) |
| Orphans > 0 | Writes happened with `foreign_keys` OFF (pragma removed?) | `internal-inventory-app-architecture-contract` invariants; check `lib/db.ts` pragmas |
| Missing satellite detail row > 0 | A row was inserted into `items` without its matching `book_details`/`clothing_details` row — likely a bypassed insert path, not something the normal API routes can produce (they insert both in one transaction) | `internal-inventory-app-failure-archaeology` — record it; `internal-inventory-app-change-control` |
| CHECK-guaranteed > 0 | Schema was weakened, or rows predate a constraint | `internal-inventory-app-failure-archaeology` — record it; `internal-inventory-app-change-control` — someone routed around gates |
...
DRIFT is not automatically bad — it means reality moved and documentation must follow: update the `internal-inventory-app-config-and-constants` ledger, then the baselines in this script, in the same change. A DRIFT on the `middleware.ts` sentinel now (i.e., it goes missing) would be BAD news — a CSRF-protection regression — the opposite of what the original 2026-07-02 version of this script treated as good news.
...
2. Never claim "the API works" from unit tests alone — see `internal-inventory-app-validation-and-qa` evidence bar (the historical D1/D2 defects shipped under a fully-green unit suite).
3. Before/after any risky operation, snapshot: `db-integrity.sh` output + row counts. Diff afterwards.
4. Write the expected output BEFORE running the probe (`internal-inventory-app-analysis-and-methodology`, predict-then-observe).

## When NOT to use this skill

- Something already failed and you need triage → `internal-inventory-app-debugging-playbook` (it uses these scripts, plus symptom mapping).
- You need to check whether a historical constraint-leak defect is really fixed → `internal-inventory-app-constraint-leak-campaign` / `internal-inventory-app-debugging-playbook` (spoiler: D1/D2/D3 already are, as of this writing).
- Running the test suite → `internal-inventory-app-validation-and-qa` (now safe to run directly from the repo root by default — see that skill for why).
- Operating/backing up → `internal-inventory-app-run-and-operate`.
...
- Scripts still target the current schema/routes, not a regression back to `books`/`/api/books`: `grep -l "FROM books\b\|/api/books" .claude/skills/internal-inventory-app-diagnostics-and-tooling/scripts/*.sh` (expect no output).

...
The shared shape pattern is `ISBN_PATTERN = /^\d{9}[\dX]$|^\d{13}$/`, duplicated in `lib/isbn.ts` and `app/api/isbn/[isbn]/route.ts` (exact constant homes → `internal-inventory-app-config-and-constants`).
...
**KNOWN LIMITATION 2 — ISBN-13 check digit never verified.** A 13-digit input only has to match `\d{13}`; `9780306406150` (bad check digit) passes straight through. Both limitations mean the Open Library lookup returning 404 is often the *first* signal of a typo. Do not "fix" these on your own — behavior change → `internal-inventory-app-change-control`.
...
Domain terms used across the internal-inventory-app skill library (infrastructure terms like WAL are deliberately excluded — see `internal-inventory-app-build-and-env` / `internal-inventory-app-run-and-operate` for those):
...
| You need… | Go to |
|---|---|
| Exact constant values / where a constant lives (limits, patterns, enum copies) | `internal-inventory-app-config-and-constants` |
| System invariants, extension costs (e.g., adding a condition grade), duplication contracts | `internal-inventory-app-architecture-contract` |
| To fix a bug or diagnose failing behavior | `internal-inventory-app-debugging-playbook`, `internal-inventory-app-diagnostics-and-tooling` |
| Details of past incidents (e.g., the gross-profit truncation) | `internal-inventory-app-failure-archaeology` |
| To change behavior this skill documents (e.g., resolve the AC3 contradiction, validate check digits) | `internal-inventory-app-change-control` |
| Build, env vars, running the app, ports | `internal-inventory-app-build-and-env`, `internal-inventory-app-run-and-operate` |
| Test strategy / QA process | `internal-inventory-app-validation-and-qa` |
| Writing docs or specs | `internal-inventory-app-docs-and-writing` |

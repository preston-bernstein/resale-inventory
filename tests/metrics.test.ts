...
// ---------------------------------------------------------------------------
// Regression coverage for the internal-scraper-service-class bug: a node-exporter
...
beforeEach(async () => {
  resetForwardAuthOutcomeCountsForTests();
  scratchDir = await mkdtemp(path.join(tmpdir(), 'internal-inventory-app-metrics-'));
...
describe('writeForwardAuthTextfile (via recordForwardAuthOutcome)', () => {
  ...
  it.skipIf(!dockerAvailable)(
    ...
    async () => {
      ...
      // Regression guard mirrored from the internal-scraper-service audit: HELP/TYPE

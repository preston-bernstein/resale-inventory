...
**Date:** 2026-07-03
**Frontier item:** `internal-inventory-app-research-frontier` item 3 ("cheapest, highest value; do this first")
**Tracked as:** DR-2 in `internal-inventory-app-failure-archaeology` (OPEN), compounds T1 (`vitest run` wipes the real DB with no backup net behind it).
...
3. **Restore drill** — the frontier item's done-milestone requires a scratch-copy restore drill validated against `db-integrity.sh` (per `internal-inventory-app-diagnostics-and-tooling` / `internal-inventory-app-validation-and-qa`). Not attempted in this session; this is an implementation-time step, not a research one.

## Why this still blocks nothing else

This item has no dependency on the AC3 Listed→Sold contradiction or on D2 (import 500s) — it is purely additive risk retirement, which is why the frontier skill ranks it first. The two design notes above (backup primitive choice, trigger cadence) are the only things a future implementer needs beyond plan.md Risk 6 as written; both should go through `internal-inventory-app-change-control` before code is written, since this is a new startup side effect.

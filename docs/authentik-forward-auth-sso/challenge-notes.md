...
## Critiques rejected
- Security Auditor's "Tailscale-LAN vs Authentik traffic ambiguous at Caddy layer" — based on a misunderstanding; Tailscale/LAN access never traverses the example.invalid Caddy block at all (separate network path), already correctly handled by the "no JWT header → pass through" branch.

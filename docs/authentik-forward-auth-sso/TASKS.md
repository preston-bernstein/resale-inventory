...
### Task 13: Fail-closed forward-auth verification (2026-08-01 security fix, post-ship)
**Status**: [x] done
**Files**: `lib/forwardAuth.ts` (return contract revised to a discriminated union), `middleware.ts` (fail-closed rejection + logging + metric), `lib/log.ts` (new), `lib/metrics.ts` (new), `app/login/page.tsx` (new `sso_error=verification_failed` banner), `docs/authentik-forward-auth-sso/requirements.md` (FR16-FR21, AC14-AC17, NFR amendment), `docs/authentik-forward-auth-sso/plan.md` (Architecture/API contract/Integration points/Risk areas updated), `docs/AUTHENTIK-DEPLOYMENT.md` (new section 5), `tests/api/forwardAuth.test.ts`, `tests/api/forwardAuthMiddleware.test.ts`, `lib/__tests__/log.test.ts` (new), `lib/__tests__/metrics.test.ts` (new), `tests/e2e/auth-pages.spec.ts`
**Test**: `npm run typecheck` clean; `npm run lint` zero new warnings/errors; `npm test` — full suite 2117 passed / 22 skipped, zero regressions; targeted coverage on the touched files (forwardAuth.ts/middleware.ts/log.ts/metrics.ts) well above the 85/80/85/85 thresholds (96.47% stmts / 94.39% branches / 100% funcs / 96.47% lines on the auth-path files together, log.ts at 100% across the board).
**Depends on**: Tasks 1-12 (this is a post-ship defect fix, not new construction)
**Parallelizable**: no
**Notes**: A 2026-08-01 fleet observability audit found that `lib/forwardAuth.ts:158-160`'s `catch { return null }` collapsed every verification-failure mode — JWKS fetch failure/timeout, key rotation, token expiry, wrong `iss`/`aud`, algorithm confusion — into one `null`, and `middleware.ts:119-122` treated `null` identically to "no credential presented," passing the request through untouched. Consequence: SSO could be silently and completely disabled (e.g. by a JWKS/Authentik outage) while the process stayed up with `NRestarts=0` and the journal emitted nothing. Fixed by: (1) `verifyAuthentikJwt` now returns `{status:'not_configured'} | {status:'verified',email} | {status:'invalid',reason,keyId?}` instead of `{email}|null`, distinguishing "no credential" from "credential presented but unverifiable"; (2) `middleware.ts` denies (401 for `/api/*`, redirect to `/login?sso_error=verification_failed` for pages) on `status:'invalid'`, uniformly across every reason so the client can never fingerprint which reason caused the rejection; (3) `lib/log.ts` (new, minimal structured logger per `internal-infra/CONVENTIONS.md` #18) logs the reason server-side, `warn` for a bad/expired/malformed credential vs. `error` for a JWKS-side failure or an unclassified thrown value; (4) `lib/metrics.ts` (new) counts outcomes by reason in-process, exportable via an optional node-exporter textfile (`NODE_EXPORTER_TEXTFILE_DIR`, inert until a separate `internal-infra` deploy change wires the real path — deploy is explicitly out of scope for this fix). `not_configured` (forward-auth disabled for this deployment entirely) keeps the original pass-through behavior — it is a deliberate posture, not a failure. Explicitly out of scope, tracked separately, not security defects: the repo's other 34 unstructured `console.error` call sites, the absent `/healthz` endpoint, and wiring the metrics textfile / Prometheus alert rule on the deploy host (all from the same 2026-08-01 audit, none of them auth-bypass-shaped).

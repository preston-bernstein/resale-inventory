...
- `lib/log.ts` (new, 2026-08-01) -- a minimal structured logger conforming to `home-infra/CONVENTIONS.md` #18's canonical log line (schema_version, ts, level, service, event, msg, plus caller fields), used by `middleware.ts` to satisfy FR19. Deliberately scoped to this fix, not a repo-wide logging migration: the 2026-08-01 fleet observability audit found 34 unstructured `console.error` call sites elsewhere in this repo, which remain out of scope here (tracked separately, not a security defect). Enforces the CONVENTIONS.md #18 redaction deny-list on its own field names as defense-in-depth, since this app is not yet onboarded to the shared Loki pipeline that would otherwise backstop it.
- `lib/metrics.ts` (new, 2026-08-01) -- in-process counters for forward-auth verification outcomes (FR20), keyed by outcome/reason. Deliberately NOT a `/metrics` HTTP route: this app fronts a public domain, and CONVENTIONS.md #18's `/metrics`-endpoint guidance assumes a scrapeable service, which nothing in this repo controls (Prometheus scrape config lives in `home-infra`, a separate deploy this fix does not touch). Instead writes an optional node-exporter textfile-collector `.prom` file, gated behind a `NODE_EXPORTER_TEXTFILE_DIR` env var that is unset (inert) until a `home-infra`-side deploy change wires it to the real host path -- see Risk areas.

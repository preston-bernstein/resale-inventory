...
Browser --> Caddy (forward_auth authentik-server:9000, copy_headers
        ...
        --> Next.js app :3000, middleware.ts (Node.js runtime, `async function
             middleware(request): Promise<NextResponse>` -- changed from sync)
              ...
              3. Forward-auth-SSO check (new, skipped per step 2 above for the
                 ...
                 c. JWT verification (jose.jwtVerify against a cached,
                    ...
                    alg:none / alg-confusion / kid not found / JWKS fetch
                    failure or timeout)?
                      -> [2026-08-01 fix] FAIL CLOSED: deny outright -- 401
                         JSON {"error":"authentik_verification_failed"} for
                         /api/*, a 302 redirect to
                         /login?sso_error=verification_failed for page
                         routes. Log the specific failure reason (warn for a
                         bad/expired/malformed credential, error for a
                         JWKS-side failure or an unclassified thrown value)
                         and increment an outcome counter for that reason,
                         per FR16-FR20. Never trusts the plaintext headers
                         (FR1, FR3, FR4, AC3) either way. This REPLACES the
                         original pass-through behavior described below,
                         which is now known to have been a defect: silently
                         passing the request through here made "JWKS
                         endpoint unreachable" indistinguishable from "no
                         credential was ever presented," which meant an
                         Authentik/JWKS outage could disable SSO fleet-wide
                         with zero log output while the process stayed
                         healthy by every other signal (2026-08-01 fleet
                         observability audit finding). The original design
                         intent -- "never trust the plaintext headers" -- is
                         unchanged; only the response to a failed-but-
                         presented credential changed, from pass-through to
                         deny.
...
**New response contract for FR17/FR18 / AC14-AC15 (2026-08-01 fix: a presented JWT that fails verification):**

- Requests under `/api/*`: `401 { "error": "authentik_verification_failed" }`.
- All other (page) requests: `302` redirect to `/login?sso_error=verification_failed`. `app/login/page.tsx`'s `SsoErrorBanner` renders a distinct, reason-agnostic banner ("We couldn't verify your SSO login. Log in with email/password below, or try again in a moment.") -- deliberately worded to never reveal which specific reason caused the rejection (FR18), unlike the server-side log line, which does distinguish it (FR19).
- This is a NEW rejection path, distinct from the existing FR8/AC4 unmatched-tenant response (`authentik_identity_unmatched` / `sso_error=unmatched`): unmatched-tenant means the credential verified successfully but no local account maps to it; verification-failed means the credential itself could not be confirmed. Client code (or a human reading the banner) does not need to tell these apart, but they are logged/counted as distinct events server-side.
...
- `lib/forwardAuth.ts` (new; return contract revised 2026-08-01, see below) -- a single lazily-initialized module-level JWKS set (`let jwksSet: ReturnType<typeof createRemoteJWKSet> | undefined`, initialized on first use from `AUTHENTIK_JWKS_URL`) -- **not** a `Map` keyed by URL; there is exactly one pinned URL for this single-IdP, single-app deployment, so a keyed cache structure is unnecessary complexity. An explicit `timeoutDuration` passed to `createRemoteJWKSet` (jose supports this option) rather than leaving the JWKS fetch on an implicit default timeout; the header-name constants (`X-Authentik-Jwt`, etc.); and the all-or-nothing env var validation described in API/interface contract above, run once at module load.

  **`verifyAuthentikJwt(jwt): Promise<ForwardAuthResult>` (2026-08-01 revision).** Originally `Promise<{ email: string } | null>`, collapsing every failure mode -- forward-auth not configured, and every possible verification failure including the verifier's own JWKS dependency being unreachable -- into one `null` that the caller could not distinguish. Revised to a discriminated union so the caller (`middleware.ts`) can and must treat "no credential" differently from "credential presented but unverifiable" (FR16):
  ```ts
  type ForwardAuthFailureReason =
    | 'jwks_unreachable' | 'key_not_found' | 'invalid_signature' | 'invalid_algorithm'
    | 'token_expired' | 'invalid_issuer' | 'invalid_audience' | 'malformed_token'
    | 'missing_email_claim' | 'unknown';
  ...
  ```
  Still never throws to the caller. `keyId` (the token's `kid` header, best-effort decoded, `undefined` if undecodable) is carried for logging only -- it is a public identifier already published in the JWKS document, not a secret, and is what makes a key-rotation-window failure diagnosable from logs alone. The classification logic (`classifyVerificationError`) maps jose's own error classes (`JWTExpired`, `JWTClaimValidationFailed`, `JOSEAlgNotAllowed`, `JWSSignatureVerificationFailed`, `JWKSNoMatchingKey`/`JWKSMultipleMatchingKeys`/`JWKSTimeout`, etc.) to the reasons above; any non-jose `Error` (a raw network/DNS failure from the underlying `fetch`) is bucketed as `jwks_unreachable`, since the JWKS fetch is the only network call in this function's call graph.
...
- `lib/log.ts` (new, 2026-08-01) -- a minimal structured logger conforming to `internal-infra/CONVENTIONS.md` #18's canonical log line (schema_version, ts, level, service, event, msg, plus caller fields), used by `middleware.ts` to satisfy FR19. Deliberately scoped to this fix, not a repo-wide logging migration: the 2026-08-01 fleet observability audit found 34 unstructured `console.error` call sites elsewhere in this repo, which remain out of scope here (tracked separately, not a security defect). Enforces the CONVENTIONS.md #18 redaction deny-list on its own field names as defense-in-depth, since this app is not yet onboarded to the shared Loki pipeline that would otherwise backstop it.
- `lib/metrics.ts` (new, 2026-08-01) -- in-process counters for forward-auth verification outcomes (FR20), keyed by outcome/reason. Deliberately NOT a `/metrics` HTTP route: this app fronts a public domain, and CONVENTIONS.md #18's `/metrics`-endpoint guidance assumes a scrapeable service, which nothing in this repo controls (Prometheus scrape config lives in `internal-infra`, a separate deploy this fix does not touch). Instead writes an optional node-exporter textfile-collector `.prom` file, gated behind a `NODE_EXPORTER_TEXTFILE_DIR` env var that is unset (inert) until a `internal-infra`-side deploy change wires it to the real host path -- see Risk areas.
...
- **(2026-08-01, RESOLVED) Silent fail-open on JWT verification failure.** This risk section originally had no entry for this because the original design treated it as intentional ("never trust the plaintext headers" -- true -- implemented as "treat exactly like no header" -- the actual defect). A 2026-08-01 fleet observability audit found the consequence: `lib/forwardAuth.ts`'s `catch { return null }` collapsed JWKS-fetch failure, timeout, key rotation, token expiry, wrong `iss`/`aud`, and algorithm-confusion into one indistinguishable `null`, and `middleware.ts` treated `null` as "no identity, pass through" -- meaning an Authentik/JWKS outage could silently disable SSO fleet-wide while the process stayed up with `NRestarts=0` and the journal emitted nothing. Fixed by the FR16-FR21 fail-closed contract above: a presented-but-unverifiable credential is now denied outright, logged per-reason, and counted. Residual, explicitly not fixed by this change: the outcome counters (FR20) are inert until a `internal-infra`-side deploy change points `NODE_EXPORTER_TEXTFILE_DIR` at the real host path and a Prometheus alert rule is added -- see `lib/metrics.ts`'s integration-point entry above. That wiring is deploy-sequenced separately from this code change.
- **This fix does NOT reopen the "deployed Caddyfile change is a hard external dependency" risk above.** That risk is specifically about the header-ABSENT case (Caddyfile forgets to forward `X-Authentik-Jwt`) -- unaffected by this fix, which only changes behavior when a JWT header IS present. A forgotten Caddyfile change still fails silently open into "header absent, behaves like local dev" exactly as documented above; this fix does not change that, and does not claim to.

...
**Test**: Call `resolveTailnetOrigin(mockRequest)` with headers containing `Host: myapp.beta.tailscale.net`, `Host: localhost`, `Host: 127.0.0.1`, `Host: lan.example.internal`; verify it returns the origin for the first, null for the rest. Also add a Vitest test file covering this behavior (see existing test patterns in the `tests/` directory for this repo's conventions).
**Depends on**: none.

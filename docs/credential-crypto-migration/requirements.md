...
10. The system shall add `"@preston-bernstein/credential-crypto": "git+ssh://nas.example.internal/srv/git/credential-crypto.git"` to `package.json` `dependencies`, installable via `npm install`, with `package-lock.json` updated to match.
...
- Dependency is fetched via `git+ssh://nas.example.internal/srv/git/credential-crypto.git` — requires the deploy/dev environment to have SSH access to `nas.example.internal` configured (per existing `~/.ssh/agent_ed25519` home-lab access) for `npm install` to succeed.
...
- Any change to the `internal-monitor-app` repo or its migrations.
...
9. `npm install` succeeds and resolves `@preston-bernstein/credential-crypto` from the `git+ssh://nas.example.internal/...` URL, with `package.json` and `package-lock.json` both reflecting the new dependency.

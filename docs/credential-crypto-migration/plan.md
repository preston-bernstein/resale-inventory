...
- `package.json` — add `"@preston-bernstein/credential-crypto": "git+ssh://nas.example.internal/srv/git/credential-crypto.git"`
  to `dependencies`.
...
- **SSH reachability to `nas.example.internal`.** `npm install` needs working SSH access
  (`~/.ssh/agent_ed25519`) to `nas.example.internal` from every environment that will run it — dev

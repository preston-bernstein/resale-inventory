...
**Test**: `npm run build` green (13 routes + Middleware; one transient `SQLITE_BUSY` on the first attempt — the known multi-worker WAL-open race from Task 19, resolved on retry, unrelated to this change; no `Backup written:` line during build confirms the NEXT_PHASE guard fired). New regression suite `lib/__tests__/backup.test.ts` (7 tests, all green) — safe to run in-repo because it takes explicit db+dbPath and works entirely in `mkdtemp` scratch dirs, never importing `lib/db` or touching the real DB: covers backup-file creation, **WAL-safety** (a row left uncheckpointed in `-wal` is captured — a bare `cp` would miss it), source DB left unmodified/undeleted, 7-copy retention, `.gitkeep` preserved during prune, same-day no-overwrite, and the NEXT_PHASE build-skip. Scratch-copy full suite (procedure B, book-seller-validation-and-qa): 146 passed | 18 skipped (was 139|18; +7 new) — and the scratch boot produced `data/backups/inventory-20260703.db` inside the scratch tree, confirming the routine runs at real startup and follows `dbPath`. Operator's real `data/inventory.db` (separate main checkout) never touched — all work in this worktree + `mktemp` scratch.

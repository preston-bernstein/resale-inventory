...
### Task 22: Fix T1 (test-wipe trap) — make DB path configurable via BOOKSELLER_DB_PATH
**Status**: [x] done (2026-07-03)
**Files**: lib/db.ts, docs/book-inventory-management/plan.md, .claude/skills/book-seller-config-and-constants/SKILL.md
**Change**: Behavior-adjacent. `lib/db.ts` now resolves its DB path as `process.env.BOOKSELLER_DB_PATH ?? path.join(process.cwd(), 'data', 'inventory.db')` — the project's first and only env-var config axis. When the var is unset, the path is byte-for-byte the historical cwd default, so production behavior is unchanged; when set, tests (and any alternate environment) can point at a throwaway DB file instead of the operator's live inventory, which is the durable fix for the T1 wipe trap (book-seller-failure-archaeology / book-seller-validation-and-qa "Candidate fix"). Documented the new axis in the same change: plan.md Integration-points entry for lib/db.ts, and the book-seller-config-and-constants ledger (Runtime/DB table row, "how to add a config axis" worked example, opening paragraph, and env-var-surface provenance one-liner, now expecting exactly one process.env hit). Did NOT touch app/api/books/[id]/status/route.ts or app/api/import/route.ts (another session works middleware on a different branch).
**Test**: `npm run build` green (13 routes; one transient SQLITE_BUSY on first attempt during page-data collection — the documented WAL-open build race, cleared on retry — not related to this change). Safe-test procedure B (scratch copy of this worktree, book-seller-validation-and-qa) with env UNSET: 139 passed | 18 skipped — matching the current baseline, confirming unchanged behavior when the var is absent. Behavioral proof of the axis: re-ran tests/integration.test.ts in the scratch copy with `BOOKSELLER_DB_PATH=<scratch>/alt-inventory.db` — the DB file was created at the override path and the default `data/inventory.db` was never created (the DELETE-wipe now lands on the override, not the default). Operator's real DB (/Users/prestonbernstein/dev/book-seller/data/inventory.db) confirmed untouched: mtime unchanged (Jul 2 13:52), read-only row count still 1. Scratch copy deleted after.
**Depends on**: none
**Parallelizable**: yes (disjoint from the middleware/DR-1 branch)
**Notes**: This axis is the intended enabler for making the skipped HTTP suite and DB-integration tests safe-by-default; a follow-up could wire `BOOKSELLER_DB_PATH` into a vitest setup file so procedure B no longer needs a full scratch copy. Not done here — out of scope; this task only adds the axis and leaves default behavior identical.

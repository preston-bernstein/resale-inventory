...
One new migration, `data/migrations/014_electronics_category.sql`, gated by `PRAGMA user_version = 14`, containing two independent pieces inside the same transaction (the rebuild needs its own transaction scope per internal-inventory-app-change-control §4; the additive `electronics_details` table can safely ride in the same file since a single migration file is already one atomic `db.transaction()` per `lib/db.ts`'s runner):

...
**What**: SSH to the desktop (`ssh desktop.example.internal`), run `sudo -u internal-inventory-app sqlite3 /home/internal-inventory-app/internal-inventory-app/data/inventory.db ".schema items"` to confirm the real current column order and CHECKs match what migration 014 assumes, and take a manual backup copy of `data/inventory.db` before migration 014 first runs against it.

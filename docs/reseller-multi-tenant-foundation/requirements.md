...
- Must preserve all existing invariants documented in `.claude/skills/internal-inventory-app-architecture-contract/SKILL.md`: integer-cents money, UUIDv4 PKs, inline CHECK constraints, the `status` state machine centralized in `lib/transitions.ts`, `category` immutability, and the "sacred DB" rules (never delete/recreate `data/inventory.db`, WAL-aware backup only, owner-only restore).
